...
from onyx.skills.bundle import inspect_skill_package
...
def inspect_current_user_skill_package(
    ...
    return SkillPackageResponse.from_inspection(inspect_skill_package(bundle_bytes))
...
def inspect_current_user_skill_candidate(
    ...
    return SkillPackageDiffResponse.from_diff(
        diff_custom_bundles(current_bundle, candidate_bundle)
    )
...
def update_current_user_skill_package_file(
    ...
    if name != skill.slug:
        raise OnyxError(
            OnyxErrorCode.INVALID_INPUT,
            f"SKILL.md frontmatter field 'name' must match the skill's "
            f"canonical name ('{skill.slug}').",
        )
    ...
    return SkillPackageResponse.from_inspection(inspect_skill_package(new_bundle))
