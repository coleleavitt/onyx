...
from onyx.auth.oidc_client import log_code_exchange_failure
...
def _build_client(provider: SSOProvider, config: dict[str, Any]) -> BaseOAuth2[Any]:
    if provider.provider_type is SSOProviderType.OIDC:
        ...
        return VerifiedEmailOpenID(
            ...
            name=provider.name,
            base_scopes=scopes,
            email_claims=config.get("email_claims"),
            require_email_verified=config.get("require_email_verified", True),
        )
...
async def oidc_login_callback_for_provider(
    ...
    try:
        ...
        log_code_exchange_failure(provider_name, e)
        raise OnyxError(
            OnyxErrorCode.VALIDATION_ERROR,
