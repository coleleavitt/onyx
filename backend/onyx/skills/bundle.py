...
def parse_skill_md_metadata(zip_bytes: bytes) -> tuple[str, str]:
    ...
    return document.metadata.name, document.metadata.description
...
def inspect_skill_package(zip_bytes: bytes) -> SkillBundleInspection:
...
def diff_custom_bundles(
    ...
    current = inspect_skill_package(current_zip_bytes)
    candidate = inspect_skill_package(candidate_zip_bytes)
